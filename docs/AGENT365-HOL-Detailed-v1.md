...
|---|---|---|
| `a365.config.example.json` | Sample agent config (tenant ID, agent name) | ❌ Will let the CLI generate this later on |
| `ToolingManifest.json` | List of tools that the agents will require | ❌ Don't manually edit, CLI to edit |
...
As part of governance and observability, Agent365 can also manage the tools that are available in your enterprise for you.

## For MCPs already approved in tenant
| Command | Purpose |
|---|---|
| `a365 setup permissions mcp --agent-name <name>` | Re-grant MCP server OAuth2 grants on the existing blueprint depending on the `ToolingManifest.json`. Also `permissions bot` / `permissions custom` / `permissions copilotstudio`. |
...
## BYO MCPs
More information [here](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/manage-tools-for-agent?view=o365-worldwide#bring-your-own-byo-mcp-server)

| Command | Purpose |
|---|---|
| `a365 develop-mcp register-external-mcp-server` | Registers an external MCP server |
| `a365 develop-mcp evaluate` | [Evaluates](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/manage-tools-for-agent?view=o365-worldwide#evaluate-mcp-servers) an mcp server |
